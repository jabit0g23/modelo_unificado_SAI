"""Telemetría: stats de modelo + escritura de métricas (idéntico al pipeline)."""
import os, csv
import pandas as pd
...
def _count_sets(m, set_names=("B", "S", "T", "G", "GRT", "GRS",
                              "B_I", "B_E", "BC", "BT", "BH", "BI",
                              "S_E", "S_I")):
    sizes = {}
    for nm in set_names:
...
def model_size_stats(m, *, breakdown=False):
    ...
    total_constr = 0
    by_constr_component = {}
    for cname, _ in _iter_constrdata(m):
        total_constr += 1
        if breakdown:
    ...
    for pname, pdat in _iter_paramdata(m):
        if _param_value_safe(pdat) is not None:
            total_params += 1
            if breakdown:
